...
export class BedrockAIService {
  ...
  constructor() {
    ...
    this.modelId = process.env.BEDROCK_MODEL_ID || 'meta.llama4-scout-17b-instruct-v1:0';
  ...
  private analyzeBehavior(userMessage: string): BehaviorMetadata {
    ...
    // Detect confidence
    const lowConfidenceIndicators = ['not sure', 'maybe', 'i think', 'confused', 'help', 'don\'t understand'];
    const highConfidenceIndicators = ['i know', 'definitely', 'sure', 'understand', 'got it', 'easy'];
    const lowConfCount = lowConfidenceIndicators.filter(phrase => lowerMessage.includes(phrase)).length;
    const highConfCount = highConfidenceIndicators.filter(phrase => lowerMessage.includes(phrase)).length;
    ...
    // Detect slang level
    const slangWords = ['bro', 'dude', 'cool', 'awesome', 'sick', 'lit', 'fire', 'vibes', 'lowkey', 'highkey', 'fr', 'ngl', 'tbh'];
    const slangCount = slangWords.filter(word => lowerMessage.includes(word)).length;
  ...
  private buildAdaptivePrompt(
    ...
    if (context.confidence === 'low') {
      prompt += 'The learner is struggling. Be supportive, patient, and encouraging. Break down concepts into smaller steps. ';
    } else if (context.confidence === 'high') {
      prompt += 'The learner is confident. Be motivating and introduce stretch challenges. Push them to think deeper. ';
    } else {
      prompt += 'The learner is progressing steadily. Maintain balanced guidance. ';
    }
    
    // Adapt based on mastery score
    if (context.masteryScore !== undefined) {
      if (context.masteryScore < 50) {
        prompt += 'Their mastery is below 50% - simplify explanations and provide more examples. ';
      } else if (context.masteryScore > 80) {
        prompt += 'Their mastery exceeds 80% - introduce advanced concepts and real-world applications. ';
      }
    }
    
    // Adapt based on slang level
    if (context.slangLevel === 'high') {
      prompt += 'Mirror their casual, slang-heavy style naturally. Use phrases like "that\'s fire", "lowkey", "ngl". ';
    } else if (context.slangLevel === 'low' || context.userTone === 'formal') {
      prompt += 'Maintain professional, formal language. Avoid slang completely. ';
    }
    ...
    prompt += 'Keep responses concise (2-4 sentences), structured, and actionable.';
   ...
   */
  private storeConversationMessage(sessionId: string, role: 'user' | 'assistant', content: string): void {
  ...
  private formatConversationHistory(messages: ConversationMessage[]): string {
    ...
    if (formatted.length > this.MAX_MEMORY_TOKENS * 4) {
      formatted = formatted.substring(formatted.length - (this.MAX_MEMORY_TOKENS * 4));
  ...
  private async invokeBedrockModel(
    ...
    try {
      const isClaudeModel = this.modelId.includes('claude') || this.modelId.includes('anthropic');
...
`;
      ...
      if (error.name === 'ServiceUnavailableException') {
        throw new Error('SERVICE_UNAVAILABLE: AI service is temporarily unavailable');
      }
      ...
      if (error.message?.includes('inference profile')) {
        throw new Error('MODEL_ACCESS_ERROR: This model requires an inference profile ARN. Try using anthropic.claude-haiku-4-5-20251001-v1:0 instead.');
      }
...
First node status="current", others="locked". Order 1,2,3... Thresholds 70-85.`;
      ...
      const response = await this.invokeBedrockModel(systemPrompt, userPrompt, 0.7, 800);
      
      // Extract JSON
      const jsonMatch = response.match(/\[[\s\S]*\]/);
      if (!jsonMatch) {
        throw new Error('PARSE_ERROR: Failed to parse roadmap from AI response');
      }

      const roadmap: RoadmapNode[] = JSON.parse(jsonMatch[0]);
      
      if (!Array.isArray(roadmap) || roadmap.length === 0) {
        throw new Error('INVALID_ROADMAP: AI returned invalid roadmap structure');
      }
  ...
  async conductCharacterAnalysis(
    ...
    try {
      const systemPrompt = 'You are a learning psychology expert. Analyze responses and return ONLY valid JSON.';
      
      const responsesText = responses.map(r => `Q: ${r.question}\nA: ${r.response}`).join('\n\n');
...
Return ONLY valid JSON:
{
  ...
  "motivation_index": 75
      ...
      const response = await this.invokeBedrockModel(systemPrompt, userPrompt, 0.5, 200);
      ...
      if (!profile.tone_type || !profile.confidence_level || typeof profile.motivation_index !== 'number') {
        throw new Error('INVALID_PROFILE: AI returned invalid profile structure');
      }
  ...
  async generateRecap(
    ...
    try {
      ...
      const systemPrompt = 'You are an encouraging learning mentor. Be brief and motivating.';
...
3. Next action`;
      ...
      const response = await this.invokeBedrockModel(systemPrompt, userPrompt, 0.7, 150);
...
Respond as the mentor. Be focused and actionable (2-4 sentences).`;
      ...
      const response = await this.invokeBedrockModel(systemPrompt, userPrompt, 0.8, 200);
  ...
  async generateStretchTask(
    ...
    try {
      const systemPrompt = 'You are an expert at creating challenging learning tasks.';
...
Provide clear task description (2-3 sentences).`;
      ...
      const response = await this.invokeBedrockModel(systemPrompt, userPrompt, 0.8, 150);
